...
class BarangKeluarController extends Controller
{
    ...
    public function viaManual(Request $request, $id)
    {
    ...
    public function viaScan(Request $request, $id)
    {
        $penjualan = Penjualan::findOrFail($id);
        $request->validate([
            'kode'     => 'required|exists:data_barang,kode_barang',
            'jumlah_keluar' => 'required|integer|min:1',
        ]);

        $barang = Data_barang::where('kode_barang', $request->kode)->firstOrFail();
        if ($request->jumlah_keluar > $barang->stok) {
            return redirect()->back()->withInput()
                ->with('swal_error', 'Stok barang ' . $barang->nama_barang . ' tidak mencukupi. Stok tersedia: ' . $barang->stok);
        }
    ...
    public function destroy($id_barang_keluar)
    {
        ...
        $id_penjualan = $barang_keluar->id_penjualan;
